...
public class Crane implements IDisplayable {
	...
	public class CraneBase implements IDisplayable {
		...
		public void drawMe(Graphics g) {
			...
			tracks = new Rectangle(truckStart - 20, truckLevel,
					truckWidth + 40, terrainLevel - truckLevel);
...
//			
		...
		public void updateLocation(double shift) {
			...
			//System.out.println("body shifted by " + body.getBounds());
//			body.translate((int) shift, 0);
//			tracks.translate((int) shift, 0);
//			fulcrum.translate((int) shift, 0);
//			
	...
	public class CraneArm implements IDisplayable {
		...
		public CraneArm() {
			...
			// anchorPoints[0] = new Point2D.Double(arms[0].x + arms[0].height /
			// 2,
			// arms[0].y + arms[0].height / 2);
			// anchorPoints[1] = new Point2D.Double(arms[1].x + arms[1].height /
			// 2,
			// arms[1].y + arms[1].height / 2);
			// anchorPoints[2] = new Point2D.Double(arms[2].x + arms[2].height /
			// 2,
			// arms[2].y + arms[2].height / 2);
			// anchorPoints[3] = new Point2D.Double(arms[3].x + arms[3].height /
			// 2,
...
//			transforms.get(0).rotate(-Math.PI / 4.0, 150, 300);
//			transforms.get(0).translate(5, -10);
//			transforms.get(1).rotate(-Math.PI / 6.0, 185, 300);
//			transforms.get(1).translate(10, -42);
//
//			transforms.get(2).rotate(-Math.PI / 8.0, 220, 300);
//			transforms.get(2).translate(-10, -65);
//			transforms.get(3).rotate(-Math.PI / 10.0, 255, 300);
//			transforms.get(3).translate(-14, -65);
			//
...
//				g2.setTransform(transforms.get(i));
	...
	public class Terrain implements IDisplayable {
		...
		public void drawMe(Graphics g) {
			...
			g2.drawLine(0, (int) (1.07f * terrainLevel), 2000,
					(int) (1.07f * terrainLevel));

			g2.setColor(greenish);
			g2.setStroke(thickStroke);
			
			g2.drawLine(0, (int) (1.00f * terrainLevel), 2000,
					(int) (1.00f * terrainLevel));
...
//		}
	...
	public void drawMe(Graphics g) {
		for (IDisplayable i : dispList) {
			...
			i.drawMe(g);
			g.translate((int) shift, 0);
			i.updateLocation(shift);
		}
	}
	...
	public void updateFrameDims(int width, int height) {
		...
		xfulcrum = new int[] { truckStart + truckWidth / 3,
				truckStart + 2 * truckWidth / 3, truckStart + truckWidth / 2 };
		yfulcrum = new int[] { truckLevel - truckMaxHeight,
				truckLevel - truckMaxHeight,
				truckLevel - truckMaxHeight - fulcrumHeight };
	}

	public void translateBody(double d, int i) {
//		System.out.println("before shifting :" + shift);
//		System.out.println("translating by " + d + " " +  i);
		shift = d - i * shift;
//		System.out.println("after shifting :" + shift + "\n");
		craneMove = true;
	}
